...
int main()
{
    ...
    control.addButton(new Button(window, {40, 380, 240, 40}, font, "Random All Colors"), new RandomAllColorsCommand(pool));
    std::vector<Command*> multiCommands;
multiCommands.push_back(new RandomAllColorsCommand(pool));
multiCommands.push_back(new RandomAllPositionsCommand(pool));
control.addButton(new Button(window, {40, 500, 240, 40}, font, "Multi Command"), 
                 new MultiCommand(multiCommands));
    while (window.isOpen())
    {
        sf::Event event;
